...
from clear_pipeline.defs.reliefweb_partitions import (
    chunks_prefix,
    country_partitions,
    datapoints_prefix,
)
...
from clear_pipeline.providers.classify import coerce_event_types
...
SYSTEM_PROMPT_TEMPLATE = (
    ...
    "  {{ value, value_low, value_high, qualifier, measure_type, "
    "  basis_period_start, basis_period_end, unit, confidence, source_quote, "
    "  chunk_index, page_number, scope_location_name, source_name, credibility "
    "  }}. `source_quote` must "
    ...
    "- MEASURE TYPE (`measure_type`): classify EVERY number by what it measures "
    "  over time, from the wording. `stock_as_of` = a point-in-time total "
    "  ('currently', 'as of', 'total displaced', 'X people are displaced'); "
    "  `period_flow` = a quantity accrued DURING a period ('newly displaced', "
    "  'during May', 'this week', 'X new cases'); `cumulative_to_date` = a "
    "  running total since an origin ('since January', 'cumulative', 'to date'). "
    "  This is the single most important classification — a flow read as a stock "
    "  (or vice-versa) mis-aggregates badly. Null only if genuinely "
    "  indeterminate.\n"
    "- VALUE RANGE + `qualifier`: capture a FINITE, usable band for every "
    "  figure — NEVER an open-ended or 0/infinity bound (an open range is "
    "  unusable downstream). Always keep `value` as the headline number. Then:\n"
    "    • EXACT ('exactly 800', a precise count): `qualifier`='exact', leave "
    "      `value_low`/`value_high` null (they default to `value`).\n"
    "    • STATED RANGE ('between 500 and 700', '500–700'): `qualifier`='approx', "
    "      `value`=a representative point, `value_low`=500, `value_high`=700.\n"
    "    • APPROXIMATION ('around', 'roughly', 'an estimated', '~600'): "
    "      `qualifier`='approx', `value`=the stated number, and a MODEST "
    "      symmetric band reflecting how vague the wording is — a firm 'an "
    "      estimated 600' → about [570, 630]; a loose 'roughly 600' → about "
    "      [500, 700].\n"
    "    • FLOOR ('more than 500', 'at least 500', 'over 500'): "
    "      `qualifier`='at_least', `value`=500, `value_low`=500 (the firm "
    "      floor), and `value_high` = a PLAUSIBLE upper bound you infer from the "
    "      report's own related figures and context — NOT infinity, NOT an "
    "      arbitrary huge number.\n"
    "    • CEILING ('up to 700', 'fewer than 700', 'nearly 700'): "
    "      `qualifier`='at_most', `value`=700, `value_high`=700 (the firm "
    "      ceiling), and `value_low` = a plausible lower bound from context — "
    "      NOT 0.\n"
    "- BASIS PERIOD (`basis_period_start`/`basis_period_end`): set these ONLY "
    "  when a figure states its OWN period, distinct from the report's overall "
    "  reporting period (e.g. '800 killed between 2 and 10 April' in a report "
    "  otherwise covering all of April) — ISO dates. Else leave BOTH null; the "
    "  report's reporting period applies to the figure.\n"
    ...
    "- SEX/AGE BREAKDOWN (`breakdown`, only on the population/need/response "
    "  figures that carry it — IDP stock, new displacements, refugees, returnee "
    "  stock, new returns, overall people-in-need, overall affected, and each "
    "  sector's people-in-need / people-targeted / people-reached): fill it ONLY "
    "  when the report disaggregates that figure by sex or age (e.g. '120,000 "
    "  displaced, of whom 52% women and 40% "
...
def extract_datapoints_for_one_report(
    *,
    ...
    # populations (clear-pipeline ADR-0001). Was overall_pin, which
