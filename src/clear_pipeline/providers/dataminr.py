"""Dataminr provider — models + poll client + Redis watermark, consolidated.

All Dataminr integration in one file (Pydantic models for the First Alert API +
the paginated poll client + the Redis last-synced watermark), matching the
providers/ single-file convention (acaps.py, hapi.py, iom_dtm.py). Ported from
clear-pipeline during the Celery→Dagster consolidation.
"""
...
# ── Models (Dataminr First Alert API, v1 format) ─────────────────────────────
