...
def test_is_first_ingest_true_when_no_reports():
    ...
    assert rw._is_first_ingest(s3, "bucket", "sdn") is True
    # It scopes the existence check to THIS country/format's report prefix.
    _, kwargs = s3.list_objects_v2.call_args
    assert kwargs["Prefix"] == rw.reports_prefix("sdn")
    assert kwargs["Bucket"] == "bucket"


def test_is_first_ingest_is_per_country():
    # A different partition's prefix — a country established elsewhere doesn't
    # make a fresh country look already-ingested.
    s3 = MagicMock()
    s3.list_objects_v2.return_value = {"KeyCount": 0}
    assert rw._is_first_ingest(s3, "bucket", "eth") is True
    _, kwargs = s3.list_objects_v2.call_args
    assert kwargs["Prefix"] == rw.reports_prefix("eth")


def test_is_first_ingest_false_when_a_report_exists():
    s3 = MagicMock()
    s3.list_objects_v2.return_value = {"KeyCount": 1}
    assert rw._is_first_ingest(s3, "bucket", "sdn") is False
